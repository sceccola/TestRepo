...
class CRM_Contribute_Form_Contribution_Confirm extends CRM_Contribute_Form_ContributionBase 
{
    ...
    public function buildQuickForm()
    {
        ...
        if ( CRM_Utils_Array::value( 'hidden_onbehalf_profile', $params ) ) {
          ...
          $session->set( 'contributetitle', $title);
          if ($title == 'Give a Gift Membership to the Trinity Forum Society')
          {
          
            $fname = $params['onbehalf']['first_name'];
            $lname = $params['onbehalf']['last_name'];
            $session->set( 'giftfname', $fname);
            $session->set( 'giftlname', $lname);
            	     
            $profileId = CRM_Core_DAO::getFieldValue( 'CRM_Core_DAO_UFGroup', 'gift_member', 'id', 'name' );
    ...
    public function postProcess( )
    {
        ...
        if ( CRM_Utils_Array::value( 'hidden_onbehalf_profile', $this->_params ) ) {
            ...
            if ($title == 'Give a Gift Membership to the Trinity Forum Society') {
     			  $orgFields = array('first_name', 'last_name');
            }
            else {
        ...
        // and it's location.
        $session = CRM_Core_Session::singleton( );
        $title = $session->get( 'contributetitle' );
        if ($title != 'Gift Membership') {
        ...
        if ( isset( $params['hidden_onbehalf_profile'] ) && isset( $behalfOrganization['organization_name'] ) ) {
	     ...
	     if ($title == 'Gift Membership') {
          $ufFields = array( );
            ...
            self::processGiftMembership( $behalfOrganization, $contactID, $this->_values, 
                                               $this->_params, $ufFields );
        }
        ...
        if ( $processMembership ) {
            ...
            if ( is_array( $paymentParams['onbehalf'] ) && !empty( $paymentParams['onbehalf'] ) ) {
             ...
             if ($title == 'Gift Membership') {
     	 		   $fieldTypes = array( 'Contact', 'Individual', 'Contribution' );
     	 		 }
     	 	else {	 
                                         ...
                                         $online = true ) 
    {
        ...
        $contribParams = array(
                               ...
                               //from 'contribution' type profile if ƒd
    ...
    // ----------
    static function processGiftMembership( &$behalfOrganization, &$contactID, &$values, &$params, $fields = null) {
        
        // formalities for creating / editing organization.
        $behalfOrganization['contact_type'] = 'Individual';
        foreach ( $behalfOrganization as $locFld => $value ) {
            if ( in_array( $locFld, array( 'phone', 'email', 'address' ) ) ) {
                $locTypeId = array_keys( $value );
                $behalfOrganization[$locFld][$locTypeId[0]]['is_primary'] = 1;
        ...
        $relType->name_a_b = 'Given Gift Membership';
        $relType->find(true);
        $relTypeId = $relType->id;
        ...
        $relParams['is_active'           ]    = 1;
            ...
            // check if matching organization contact exists
            require_once 'CRM/Dedupe/Finder.php';
            $dedupeParams = CRM_Dedupe_Finder::formatParams($behalfOrganization, 'Individual');
            $dedupeParams['check_permission'] = false;
            ...
            if ( count($dupeIDs) >= 1 ) {
                $behalfOrganization['contact_id'] = $dupeIDs[0];
                // don't allow name edit
                unset($behalfOrganization['first_name']);
                unset($behalfOrganization['last_name']);
      
            }
            else
            {
      			$session = CRM_Core_Session::singleton( );
        		 $gfname = $session->get( 'giftfname' );
        	     $glname = $session->get( 'giftlname' );
       
     		    $behalfOrganization[first_name] = $gfname;
         		$behalfOrganization[last_name] = $glname;
         	}	
        ...
        $relParams['contact_check'][$orgID] = 1; 
        $startDate = date('Y-m-d');
        $relParams['start_date'] = $startDate;
        $date  = explode('-', $startDate );
        $year  = $date[0];
                $month = $date[1];
                $day   = $date[2];
                $year = $year +1;
        //         $endDate = date('Y-m-d H:i:s',mkDate($month, $day, $year));
               
                
                  
   //     $relParams['end_date'] = $endDate;        
        $cid = array( 'contact' => $contactID );
        $relationship = CRM_Contact_BAO_Relationship::create($relParams, $cid);
       
        
             // required for mailing/template display ..etc 
            $values['related_contact'] =$orgID; //  $contactID;
            // required for IPN
            $params['related_contact'] =$orgID; // $contactID;
